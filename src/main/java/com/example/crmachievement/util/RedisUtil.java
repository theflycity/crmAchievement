package com.example.crmachievement.util;
...
public class RedisUtil {
     ...
     *
     * @param key      键
     * @param time     时间(TimeUnit) 注意:这里将会替换原有的时间
     * @param timeUnit 单位
     */
    ...
    public List<String> scan(String pattern) {
        ...
        try {
            ...
            log.error("Scan operation failed", e);
            return Collections.emptyList();
        } finally {
            // 确保无论try/catch是否抛出异常，finally都会执行资源释放
            try {
                if (cursor != null) cursor.close(); // 显式关闭Cursor资源
            } finally {
                // 双重try确保即使前一步关闭失败，仍会尝试释放连接
                try {
                    RedisConnectionUtils.releaseConnection(rc, factory);
                } catch (Exception e) {
                    log.error("Failed to release Redis connection", e);
                }
            }
        }
    }
